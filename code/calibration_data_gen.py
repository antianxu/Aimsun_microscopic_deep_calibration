...
# For aimsun 8.4
# for SITEPACKAGES in ["C:\Python27\\Lib\\site-packages",
# 					 "C:\Python27\\Lib",
# 					 "C:\Python27\\DLLs"]:
# 	if SITEPACKAGES not in sys.path:
# 		sys.path.append(SITEPACKAGES)
...
#from metro3.metro.tools import tools
...
class CalibrationDataModel(aimsunModel):
	...
	def __init__(self, 
				...
				console=None, 
				model=None):
		super(CalibrationDataModel, self).__init__(iniFile, replicationId, modelType, 
											console, model, calculateLOS=False, baseCaseModel=False)
	...
	def run(self):
		...
		if self._model is not None:
			...
			if replication is None or not replication.isA("GKReplication"):
				...
				#self._logger.debug('Number of vehicles reached their destination = %f\n'
	...
	def setParams(  self,
		...
		# if roadTypes != None:
		# 	for roadType in roadTypes.itervalues():
...
if __name__ == "__main__":
	...
	argParser.add_argument('-w', "--write_to_csv", action='store_true',
                            help="Write Aimsun output to csv or print to stdout")
	...
	#tools.setup_logging(args.log_level, path=os.path.join(baseDir, 'logging.yaml'))
	...
	if args.write_to_csv is True:
		...
		seperator = ' '
		for row in output:
			print(seperator.join(str(n) for n in row))
